...
#Source: http://colorbrewer2.org. 5 data classes, diverging nature of data, 8t color scheme  
cols <- c("#1a9641","#a6d96a","#ffff4f","#fdae61","#d7191c") 
...
```{r, message=FALSE, fig.height = 4, fig.width = 11}
...
ggplot(data = classifications, aes(x = Classification, y = Quantity, fill = Type)) +
 ...
 coord_flip()+
 scale_fill_manual(values = c(cols[5],cols[5], cols[5],cols[1], cols[1],cols[1], cols[5],cols[1]))+
  geom_text(aes(label=percent(Quantity)),size = 4, position = position_stack(vjust = 0.5))+
  scale_x_discrete(limits = positions)+
  homeless_theme +
  theme(legend.position="none")+
  labs(title = "Majority of homeless are temporary,\nsheltered, alone and male",
       subtitle = "Data ratifies homelessness is not a permanent experience\nfor most people", caption = "Source: U.S. Department of Housing and Urban Development (HUD)",x = "", y="Percentage")
...
waffle <- 
  waffle(homeless_proportion*10, rows=20, size=0.6, colors=cols,  xlab="1 square is 0.1% of homeless population")+
  homeless_theme+
...
parall_coord <- 
  ggparcoord(data = races_proportions,
    ...
    scale_color_manual(values = c("White"=cols[1], "Black or African American"=cols[2],"Native American"=cols[3],"Asian"=cols[4],"Other races"=cols[5]),breaks=c("White", "Black or African American","Native American","Asian","Other races"))+
...
```{r, warning=F, message=FALSE, fig.height = 4, fig.width = 11}
...
```{r, fig.height = 5, fig.width = 11, results='hide', fig.keep='all', warning = F, message = F}
...
```{r, fig.height = 7, fig.width = 7, results='hide', fig.keep='all', warning = F, message = F}
